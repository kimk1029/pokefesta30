...
async function loadStats() {
  try {
    ...
    const [users, feedsAll, feedsToday, reportsAll, trades, messagesAll, unread] = await Promise.all([
      prisma.user.count(),
      prisma.feed.count(),
...
export default async function Page() {
  ...
  const { stats, recentFeeds, recentUsers } = data;

  return (
    <>
      <h1 className="admin-h1">대시보드</h1>
      <p className="admin-sub">전체 운영 현황 요약</p>
              ...
              <tbody>
                {recentFeeds.map((f) => (
                  <tr key={f.id}>
                    ...
                    <td>
                      <span className={`tag ${f.kind === 'report' ? 'tag-report' : 'tag-general'}`}>
                        {f.kind}
                      </span>
                    </td>
