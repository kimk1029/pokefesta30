            ...
            {NOTICES.length === 0 ? (
              <PixelFrame bg={colors.white} borderWidth={2} shadow={3} hi={null} lo={null}>
                <View style={{ padding: 30, alignItems: 'center' }}>
                  <PixelText variant="ko" size={10} color={colors.ink3}>등록된 공지가 없어요</PixelText>
                </View>
              </PixelFrame>
            ) : (
    ...
    <PixelFrame bg={colors.white} borderWidth={2} shadow={3} hi={null} lo={null}>
      <View style={{ padding: 14 }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 6 }}>
          {tag ? (
            <View style={{ paddingHorizontal: 7, paddingVertical: 2, backgroundColor: tag.bg, borderColor: colors.ink, borderWidth: 1 }}>
              <PixelText variant="pixel" size={8} color={tag.fg} style={{ letterSpacing: 0.5 }}>{tag.label}</PixelText>
            </View>
          ) : null}
          <PixelText variant="pixel" size={8} color={colors.ink3} style={{ letterSpacing: 0.3 }}>{notice.date}</PixelText>
        </View>
        <PixelText variant="ko" size={12} weight="bold" color={colors.ink} style={{ marginBottom: 8, letterSpacing: 0.5 }}>{notice.title}</PixelText>
        <PixelText variant="ko" size={10} color={colors.ink2} style={{ lineHeight: 18 }}>{notice.body}</PixelText>
      </View>
    </PixelFrame>
  );
}
