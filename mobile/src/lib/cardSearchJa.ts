import { POKEMON_KO_TO_JA } from './pokemonNamesKoJa';

/**
 * 한국어 검색어 → 일본어 표기 변환 (snkrdunk 검색용).
 *
 * PokeAPI 전 1025종(POKEMON_KO_TO_JA) + KR/JP 공식 표기가 다르거나 구어체라
 * 자동 사전에 없는 트레이너/별칭 + 자주 쓰는 카드 용어를 합쳐 매핑한다.
 * 웹 `src/lib/cardTranslate.ts` 의 KO→JA 매핑(POKEMON + CHARACTERS)과 정렬.
 */

/** KR/JP 표기가 다르거나 구어체라 종족 사전에 없는 별칭. (웹 CHARACTERS 와 동기화) */
const EXTRA_KO_TO_JA: Record<string, string> = {
  // 트레이너/등장인물 — KR/JP 표기 상이
  명희: 'メイ',
  메이: 'メイ',
  휴이: 'キョウヘイ',
  모야모: 'ナンジャモ', // Iono (SV 전기 체육관) — KR 공식명
  모란: 'ボタン', // Penny — KR 공식명 '모란'
  난천: 'シロナ',
  풀잎: 'ナタネ',
  단지: 'デンジ',
  태홍: 'アカギ',
  진철: 'シンジ',
  미카: 'カトレア',
  벨라: 'ベル',
  체렌: 'チェレン',
  스즈나: 'スズナ',
  미스즈: 'ミタル',
  리리에: 'リーリエ',
  구즈마: 'グズマ',
  루자미네: 'ルザミーネ',
  구라이브: 'グラジオ',
  페퍼: 'ペパー',
  아오키: 'アオキ',
  보탄: 'ボタン',
  하사크: 'ハッサク',
  미모사: 'ミモザ', // Miriam — SV 아카데미 양호교사
  // 인기 서포터 카드 *전체 이름* (이름+효과) — 긴 키부터 매칭되어 통째로 정확 변환
  '릴리에의 전력': 'リーリエの全力', // Lillie's Full Force
  '난천의 패기': 'シロナの覇気', // Cynthia's Ambition
  '마리의 프라이드': 'マリィのプライド', // Marnie's Pride
  '보스의 지령': 'ボスの指令', // Boss's Orders
  '박사의 연구': '博士の研究', // Professor's Research
  // 인기 서포터/트레이너 개인명 (웹 cardTranslate 와 동기화)
  릴리에: 'リーリエ', // Lillie
  마리: 'マリィ', // Marnie
  플라타느박사: 'プラターヌ博士', // Sycamore
  쿠쿠이박사: 'ククイ博士', // Kukui
  주박사: 'アララギ博士', // Juniper
  올림박사: 'オーリム博士', // Sada
  투로박사: 'フトゥー博士', // Turo
  이슬: 'カスミ', // Misty
  그린: 'グリーン', // Blue
  레드: 'レッド', // Red
  블루: 'ブルー', // Leaf
  카렌: 'カリン', // Karen
  벨: 'ベル', // Bianca
  카밀레: 'カミツレ', // Elesa
  사나: 'サナ', // Shauna
  티에르노: 'ティエルノ', // Tierno
  트로바: 'トロバ', // Trevor
  카르네: 'カルネ', // Diantha
  하우: 'ハウ', // Hau
  글라디오: 'グラジオ', // Gladion
  일리마: 'イリマ', // Ilima
  키아웨: 'カキ', // Kiawe
  수련: 'スイレン', // Lana
  아세로라: 'アセロラ', // Acerola
  할라: 'ハラ', // Hala
  '구즈마&할라': 'グズマ&ハラ', // Guzma & Hala
  말리화: 'マツリカ', // Mina
  나누: 'クチナシ', // Nanu
  라이치: 'ライチ', // Olivia
  비케: 'ビッケ', // Wicke
  플루메리: 'プルメリ', // Plumeria
  호브: 'ホップ', // Hop
  비트: 'ビート', // Bede
  소니아: 'ソニア', // Sonia
  야청: 'ルリナ', // Nessa
  채두: 'サイトウ', // Bea
  어니언: 'オニオン', // Allister
  순무: 'カブ', // Kabu
  포플러: 'ポプラ', // Opal
  금랑: 'キバナ', // Raihan
  두송: 'ネズ', // Piers
  올리브: 'オリーヴ', // Oleana
  멜론: 'メロン', // Melony
  피오니: 'ピオニー', // Peony
  클라벨: 'クラベル', // Clavell
  지니어: 'ジニア', // Jacq
  테사: 'オモダカ', // Geeta
  단풍: 'カエデ', // Katy
  콜사: 'コルサ', // Brassius
  곤포: 'ハイダイ', // Kofu
  청목: 'アオキ', // Larry
  라임: 'ライム', // Ryme
  리파: 'リップ', // Tulip
  그루샤: 'グルーシャ', // Grusha
  카지: 'スグリ', // Kieran
  시유: 'ゼイユ', // Carmine
  // 콜라보 일러스트레이터/브랜드 — snkrdunk 는 영문 'YU NAGABA' 로 노출되므로
  // ja 변환 결과를 그대로 영문으로.
  유나가바: 'YU NAGABA',
  나가바: 'YU NAGABA',
  // 클래식 트레이너 + 구어체 별칭 (snkrdunk 트레이너/구판 카드 검색용)
  이슬이: 'カスミ', // Misty
  비주기: 'サカキ', // Giovanni
  냐옹: 'ニャース', // Meowth 구어체 (공식명 '나옹'은 종족 사전)
  로켓단: 'ロケット団',
  한지우: 'サトシ', // Ash
  // 관동 체육관 관장
  마티스: 'マチス', // Lt. Surge
  민화: 'エリカ', // Erika
  독수: 'キョウ', // Koga
  강연: 'カツラ', // Blaine
  초련: 'ナツメ', // Sabrina
  // 애니 로켓단 삼인방 + 라이벌/동료
  로사: 'ムサシ', // Jessie
  로이: 'コジロウ', // James
  웅: 'タケシ', // Brock
  오바람: 'シゲル', // Gary
  봄이: 'ハルカ', // May
  빛나: 'ヒカリ', // Dawn
  세레나: 'セレナ', // Serena
  고우: 'ゴウ', // Goh
  // 챔피언 / 빌런 두목 / 박사
  목호: 'ワタル', // Lance
  단델: 'ダンデ', // Leon
  게치스: 'ゲーチス', // Ghetsis
  플라드리: 'フラダリ', // Lysandre
  오박사: 'オーキド', // Prof. Oak
  // 빌런 두목
  마적: 'マツブサ', // Maxie (마그마단)
  아강: 'アオギリ', // Archie (아쿠아단)
  로즈: 'ローズ', // Rose (마크로코스모스)
  // 챔피언
  윤진: 'ミクリ', // Wallace (호연 챔피언/루네 관장)
  노간주: 'アデク', // Alder
  백연: 'カルネ', // Diantha
  네모: 'ネモ', // Nemona
  // 칼로스/하나/알로라 동료·캡틴
  시트론: 'シトロン', // Clemont
  유리카: 'ユリーカ', // Bonnie
  아이리스: 'アイリス', // Iris
  덴트: 'デント', // Cilan
  마오: 'マオ', // Mallow
  스이렌: 'スイレン', // Lana
  카키: 'カキ', // Kiawe
  마마네: 'マーマネ', // Sophocles
  코하루: 'コハル', // Chloe
  // 성도 체육관 관장
  비상: 'ハヤト', // Falkner
  호일: 'ツクシ', // Bugsy
  꼭두: 'アカネ', // Whitney
  유빈: 'マツバ', // Morty
  사도: 'シジマ', // Chuck
  규리: 'ミカン', // Jasmine
  류옹: 'ヤナギ', // Pryce
  이향: 'イブキ', // Clair
  // 호연 체육관 관장 + 챔피언
  원규: 'ツツジ', // Roxanne
  철구: 'トウキ', // Brawly
  암페어: 'テッセン', // Wattson
  민지: 'アスナ', // Flannery
  종길: 'センリ', // Norman
  은송: 'ナギ', // Winona
  아단: 'アダン', // Juan
  성호: 'ダイゴ', // Steven
  // 신오 체육관 관장
  강석: 'ヒョウタ', // Roark
  유채: 'ナタネ', // Gardenia
  자두: 'スモモ', // Maylene
  맥실러: 'マキシ', // Crasher Wake
  멜리사: 'メリッサ', // Fantina
  동골: 'トウガン', // Byron
  // 원피스 카드게임 — 밀짚모자 일당 + 인기 캐릭 (웹 cardTranslate ONE_PIECE 와 동기화)
  '몽키 D. 루피': 'モンキー・D・ルフィ',
  '몽키 D 루피': 'モンキー・D・ルフィ',
  루피: 'ルフィ',
  '롤로노아 조로': 'ロロノア・ゾロ',
  조로: 'ゾロ', // 조로아/조로아크는 더 길어 우선 매칭
  나미: 'ナミ',
  우솝: 'ウソップ',
  상디: 'サンジ',
  산지: 'サンジ',
  '토니토니 쵸파': 'トニートニー・チョッパー',
  쵸파: 'チョッパー',
  초파: 'チョッパー',
  '니코 로빈': 'ニコ・ロビン',
  로빈: 'ロビン',
  프랑키: 'フランキー',
  브룩: 'ブルック',
  징베: 'ジンベエ',
  에이스: 'エース', // 에이스번(cinderace)은 더 길어 우선 매칭
  샹크스: 'シャンクス',
  '트라팔가 로': 'トラファルガー・ロー',
  트라팔가로: 'トラファルガー・ロー',
  '보아 한코크': 'ボア・ハンコック',
  한코크: 'ハンコック',
  야마토: 'ヤマト',
  우타: 'ウタ',
  카이도: 'カイドウ', // '홋카이도'(지명)는 더 길어 우선 매칭
  원피스: 'ワンピース',
  // 자주 쓰는 카드 용어
  메가: 'メガ',
  카드게임: 'カードゲーム',
  카드: 'カード',
  프로모: 'プロモ',
  // 이로치(色違い) = shiny. 커뮤니티 줄임말 "이로치" → 일본어 色違い (웹 cardTranslate 와 동기화)
  이로치: '色違い',
  '색이 다른': '色違い',
  색이다른: '色違い',
  // 샤이니 = シャイニー. 세트명 "샤이니 트레저/스타 V" 는 더 길어 KEYS 정렬상 우선 매칭됨.
  샤이니: 'シャイニー',
  // 태그팀(SM 시기 2인 1카드 기믹) — 일본은 タッグチーム 표기 (웹 cardTranslate 와 동기화)
  태그팀: 'タッグチーム',
  '태그 팀': 'タッグチーム',
  // 박스/확장팩 용어
  박스: 'BOX',
  팩: 'パック',
  부스터: 'ブースター',
  확장팩: '拡張パック',
  강화확장팩: '強化拡張パック',
  '강화 확장팩': '強化拡張パック',
  하이클래스팩: 'ハイクラスパック',
  '하이클래스 팩': 'ハイクラスパック',
  덱빌드박스: 'デッキビルドBOX',
  포켓몬센터: 'ポケモンセンター',
  // 세트(확장팩) 이름 — 박스가 이 이름으로 판매됨 (cardNameKo JA→KO 의 역)
  '배틀 파트너즈': 'バトルパートナーズ',
  '로켓단의 영광': 'ロケット団の栄光',
  '열풍의 아리나': '熱風のアリーナ',
  '테라스타르 페스티벌': 'テラスタルフェスティバル',
  '슈퍼일렉트릭 브레이커': '超電ブレイカー',
  '초전 브레이커': '超電ブレイカー',
  '낙원 드라고나': '楽園ドラゴーナ',
  화이트플레어: 'ホワイトフレア',
  블랙볼트: 'ブラックボルト',
  '스텔라 미라클': 'ステラミラクル',
  '나이트 원더러': 'ナイトワンダラー',
  '변환의 가면': '変幻の仮面',
  '크림슨 헤이즈': 'クリムゾンヘイズ',
  '사이버 저지': 'サイバージャッジ',
  '와일드 포스': 'ワイルドフォース',
  '샤이니 트레저': 'シャイニートレジャー',
  '미래의 일섬': '未来の一閃',
  '고대의 포효': '古代の咆哮',
  '레이징 서프': 'レイジングサーフ',
  '흑염의 지배자': '黒炎の支配者',
  클레이버스트: 'クレイバースト',
  스노해저드: 'スノーハザード',
  '트리플렛 비트': 'トリプレットビート',
  'VSTAR 유니버스': 'VSTARユニバース',
  '패러다임 트리거': 'パラダイムトリガー',
  '로스트 어비스': 'ロストアビス',
  'VMAX 클라이맥스': 'VMAXクライマックス',
  '이브이 히어로즈': 'イーブイヒーローズ',
  '샤이니 스타 V': 'シャイニースターV',
  '칠흑의 가이스트': '漆黒のガイスト',
  '백은의 랜스': '白銀のランス',
  '경천의 볼테카': '仰天のボルテッカー',
  '일격 마스터': '一撃マスター',
  '연격 마스터': '連撃マスター',
  '쌍벽의 파이터': '双璧のファイター',
  '마천 퍼펙트': '摩天パーフェクト',
  '창공 스트림': '蒼空ストリーム',
  '퓨전 아츠': 'フュージョンアーツ',
  타임게이저: 'タイムゲイザー',
  '스페이스 저글러': 'スペースジャグラー',
  // 일본 지명(한자) — 지역 한정/포켓몬센터 프로모 검색
  홋카이도: '北海道', // '카이도'(원피스 Kaido) 오매칭 방지 겸용
  후쿠오카: '福岡',
  토호쿠: '東北',
  히로시마: '広島',
  오사카: '大阪',
  도쿄: '東京',
  삿포로: '札幌',
  나고야: '名古屋',
  요코하마: '横浜',
  센다이: '仙台',
  교토: '京都',
  고베: '神戸',
  오키나와: '沖縄',
};

// 종족 사전(전수) + 별칭(override) 병합. 긴 키부터 매칭해 부분어 충돌 방지.
const MAP: Record<string, string> = { ...POKEMON_KO_TO_JA, ...EXTRA_KO_TO_JA };
const KEYS = Object.keys(MAP).sort((a, b) => b.length - a.length);

/**
 * 검색어의 알려진 한국어 토큰을 일본어 표기로 치환. 모르는 부분은 원형 유지.
 * 예) "비주기" → "サカキ", "리자몽ex" → "リザードンex", "메가리자몽" → "メガリザードン".
 */
export function koToJaSearch(text: string): string {
  let out = (text ?? '').trim();
  if (!out) return out;
  for (const k of KEYS) {
    if (out.includes(k)) out = out.split(k).join(MAP[k]);
  }
  return out;
}
