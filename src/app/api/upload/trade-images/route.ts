...
/**
 * 거래글 첨부 사진 업로드.
 *  - multipart/form-data: files[] (최대 5장)
 *  - 로그인 필수
 *  - @vercel/blob put() 으로 public 업로드 → URL 반환
 *  - 클라이언트가 이미 압축해서 보내는 전제 (~200KB/장 목표)
 *
 * 필수 Vercel 환경변수: BLOB_READ_WRITE_TOKEN
 *   Vercel 대시보드 → Storage → Create (Blob store) → 프로젝트 연결 시 자동 주입됨.
 */

const MAX_FILES = 5;
const MAX_BYTES = 4 * 1024 * 1024; // 압축했어도 안전 상한 4MB/장
...
export async function POST(req: NextRequest) {
  ...
  try {
    for (const file of files) {
      ...
      const pathname = `trade/${session.user.id}/${Date.now()}-${Math.random().toString(36).slice(2, 8)}.${ext}`;
    ...
    console.error('[upload.trade-images]', msg);
