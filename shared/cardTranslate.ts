/**
 * 카드 도메인 한/영/일 단일 매퍼 — 웹·모바일·NAS 서버가 전부 이 파일 하나를 쓴다.
 *
 * 사전은 작품(게임)별 Term[] 로 구획한다: POKEMON*, ONE_PIECE, 공용(CARD_TERMS,
 * SETS_PLACES). 새 작품(나루토 등)은 배열 하나 추가 + TERMS_BY_GAME/ALL 등록이 전부다.
 * 같은 데이터에서 세 방향이 모두 파생된다:
 * - 스니덩(일본) 검색: translate(text, 'ja')          … KO→JA
 * - eBay(영미권) 검색: translate(text, 'en')          … KO→EN
 * - 결과 표시 한국어화: translateKnownCardNameToKo()  … JA→KO (역방향 자동 생성)
 *
 * 소비처: 웹 src/lib/cardTranslate.ts(shim), NAS server/routes/cardLang.ts(@/lib 경유),
 *         모바일 cardSearchJa/cardNameKo(오프라인 폴백 shim).
 *
 * 정책:
...
const CARD_TERMS: Term[] = [
  ...
  // 같은 ja 키는 마지막 항목이 JA→KO 역방향을 이김 — 통용 표기 '이로치'를 뒤에.
  ...
  // 태그팀(SM 시기 2인 1카드 기믹) — 일본은 タッグチーム 표기. JA→KO 역방향은
  // 뒤에 둔 무공백 '태그팀' 이 이기도록 공백 변형을 앞에 둠.
  { ko: '태그 팀',   en: 'tag team',    ja: 'タッグチーム' },
  { ko: '태그팀',    en: 'tag team',    ja: 'タッグチーム' },
  ...
  { ko: '하이퍼볼',  en: 'ultra ball',  ja: 'ハイパーボール' },
  { ko: '슈퍼볼',    en: 'great ball',  ja: 'スーパーボール' },
  { ko: '프레셔스볼', en: 'cherish ball', ja: 'プレシャスボール' },
  { ko: '상처약',    en: 'potion',      ja: 'きずぐすり' },
  { ko: '좋은 상처약', en: 'super potion', ja: 'すごいきずぐすり' },
  { ko: '스타터 세트', en: 'starter set', ja: 'スターターセット' },
  { ko: '스타터',    en: 'starter',     ja: 'スターター' },
  { ko: '덱',        en: 'deck',        ja: 'デッキ' },
  { ko: '싱글',      en: 'single',      ja: 'シングル' },
  { ko: '챔피언',    en: 'champion',    ja: 'チャンピオン' },
  { ko: '패러렐',    en: 'parallel',    ja: 'パラレル' }, // 원피스 카드 패러렐 레어 표기

  // 매물 상태·거래 용어 (구 모바일 cardNameKo 사전 통합) — JA→KO 표시가 주 용도지만
  // KO→JA 검색("미개봉 151 박스")에도 쓰인다. 짧은 키는 긴 키(longest-first)가 보호.
  { ko: '미개봉',    ja: '未開封' },
  { ko: '개봉됨',    ja: '開封済' },
  { ko: '개봉',      ja: '開封' },
  { ko: '새상품',    ja: '新品' },
  ...
  { ko: '평품',      ja: '並品' },
  { ko: '정크(불량)', ja: 'ジャンク' },
  { ko: '슈링크 있음', ja: 'シュリンク付き' },
  { ko: '슈링크 있음', ja: 'シュリンクあり' },
  { ko: '슈링크 없음', ja: 'シュリンクなし' },
  { ko: '감정 완료', ja: '鑑定済み' },
  { ko: '감정품',    ja: '鑑定品' },
  { ko: '감정',      ja: '鑑定' },
  { ko: '한정품',    ja: '限定品' },
  ...
  { ko: '배송비 포함', ja: '送料込み' },
  { ko: '배송비 포함', ja: '送料込' },
  { ko: '즉시 구매 OK', ja: '即購入OK' },
  { ko: '즉시 구매 가능', ja: '即購入可' },
  { ko: '즉시 발송', ja: '即発送' },
  { ko: '흠집 있음', ja: 'キズあり' },
  { ko: '흠집 없음', ja: 'キズなし' },
  { ko: '접힘',      ja: '折れ' },
  { ko: '백카케(테두리 까짐)', ja: '白かけ' },
  { ko: '긁힘',      ja: '擦れ' },
  { ko: '포함',      ja: '付き' },
  { ko: '없음',      ja: 'なし' },
  { ko: '있음',      ja: 'あり' },
...
import { POKEMON_KO_TO_JA } from './data/pokemonNamesKoJa';
...
const CHARACTERS: Term[] = [
  ...
  // 구 모바일 cardNameKo 사전에서 통합한 인물들
  { ko: '카이',     ja: 'カイ',         en: 'Irida' },          // 히스이 진주단
  { ko: '아카마츠', ja: 'アカマツ',     en: 'Crispin' },
  { ko: '키하다',   ja: 'キハダ',       en: 'Dendra' },         // SV 배틀 교사
  { ko: '시키미',   ja: 'シキミ',       en: 'Shauntal' },       // 하나 4천왕
  { ko: '시바',     ja: 'シバ',         en: 'Bruno' },          // 관동 4천왕
  { ko: '아크로마', ja: 'アクロマ',     en: 'Colress' },
  { ko: '볼 가이',  ja: 'ボールガイ',   en: 'Ball Guy' },
  { ko: '볼가이',   ja: 'ボールガイ',   en: 'Ball Guy' },
  { ko: '오림',     ja: 'オーリム',     en: 'Sada' },           // 올림박사(オーリム博士)는 더 길어 우선
  { ko: '후투',     ja: 'フトゥー',     en: 'Turo' },           // 투로박사(フトゥー博士)는 더 길어 우선
...
const SETS_PLACES: Term[] = [
  ...
  { ko: '다크 판타즈마', ja: 'ダークファンタズマ' },
  { ko: '소드&실드', ja: 'ソード&シールド' },
  { ko: '소드&실드', ja: 'ソード＆シールド' }, // 전각 ＆ 변형 — 역방향 겸용
  { ko: '포켓몬 GO', ja: 'ポケモンGO' },
...
/**
 * 작품(게임)별 사전 구획 — 새 작품 추가 시 여기와 ALL 에만 등록하면
 * KO→JA/EN 검색·JA→KO 표시 세 방향이 전부 자동 파생된다.
 * 배열 안 순서 규칙: 같은 ja 키가 여럿이면 *마지막* 항목이 JA→KO 표시를 이긴다.
 */
export const TERMS_BY_GAME: Record<string, Term[]> = {
  pokemon: [...POKEMON, ...CHARACTERS, ...TRAINERS_EXTRA, ...SUPPORTER_CARDS],
  onepiece: ONE_PIECE,
  // naruto: NARUTO,  ← 새 작품은 이런 식으로 추가
  common: [...CARD_TERMS, ...SETS_PLACES], // 레어도·상품·상태·세트명·지명 등 전 작품 공용
};

// 주의: 스프레드 순서가 곧 사전 우선순위(뒤가 역방향 우선) — 기존 동작 유지를 위해
// POKEMON → CARD_TERMS → CHARACTERS → ONE_PIECE → TRAINERS_EXTRA → SUPPORTER → SETS 순.
